// assets/js/comparison.js - Logica per la visualizzazione e interazione della tabella di confronto
...
export function renderComparisonTable(fileProducts, shopifyProducts) {
    ...
    if (!comparisonTableContainer || !tableContentPlaceholder || !approveSelectedBtn || !approveAllBtn) {
        console.error("Contenitori della tabella di confronto o bottoni di approvazione non trovati.");
        return;
    }
    ...
    const allRelevantProducts = new Set(); // Per tenere traccia di tutti i minsan da processare

    // Aggiungi tutti i minsan dal file
    fileProducts.forEach(p => allRelevantProducts.add(String(p.Minsan).trim()));
    // Aggiungi tutti i minsan da Shopify che non sono nel file (potrebbero essere da azzerare)
    shopifyProducts.forEach(p => {
        const minsanKey = String(p.minsan).trim();
        if (!allRelevantProducts.has(minsanKey) && (p.variants[0]?.inventory_quantity ?? 0) > 0) {
            allRelevantProducts.add(minsanKey);
        }
    });

    // Ordina i minsan per una visualizzazione coerente
    const sortedMinsans = Array.from(allRelevantProducts).sort();
...
export function showProductPreviewModal(minsan, fileProduct, shopifyProduct, type) {
    ...
    if (!modalTitle || !diffTbody || !approveBtn || !modalOverlay) {
        console.error("Elementi della modal di anteprima non trovati. Assicurati che 'preview-modal.html' sia caricato.");
        return;
    }

    modalTitle.textContent = `Anteprima Modifiche per ${minsan}`;
    ...
    const newApproveBtn = document.getElementById('preview-modal-approve-btn'); // Recupera il nuovo bottone
    ...
    newApproveBtn.addEventListener('click', (e) => {
        ...
        showUploaderStatus(`Approva singola dalla modal per ${targetMinsan} (Azione: ${action}) - Implementazione dell'invio API richiesta`, 'info');
